...
SYSTEM_PROMPT = """
You are CodeSunny AI Assistant - a helpful, professional, and non-pushy assistant.
...
- Use appropriate greetings: "Namaste" (Hindi), "Hello" (English), "Hey" (Hinglish)

Core Rules:
1. Understand user intent FIRST before responding
2. Generate demos/images when requested
3. Perform SEO audits when asked
4. Only discuss pricing when user asks about cost
5. Do NOT push sales unnecessarily
6. Keep responses clear, short (3-5 sentences max)
7. Maintain conversation context
8. Be helpful and professional
9. Never repeat marketing lines unless relevant
10. If tool is required, suggest it naturally
11. If user asks for a landing page demo/preview, do not switch to pricing flow
12. Ask at most 1 clarifying question before showing a demo
...
def generate_response_with_groq(message: str, history: list, intent: str) -> str:
...
def generate_response_with_gemini(message: str, history: list, intent: str) -> str:
    """
    ...
    if not intent_model:
        if groq_client:
            return generate_response_with_groq(message, history, intent)
        return "I'm here to help! How can I assist you today?"
    ...
    try:
        ...
        conversation_parts = [
            ...
            {
                "role": "model",
                "parts": ["Understood. I'll be helpful and professional without being pushy, and I'll respond in the same language the user uses."],
            },
        ]
        ...
        chat = intent_model.start_chat(history=conversation_parts)
