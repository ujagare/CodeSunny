...
class GeminiModelCompat:
    def __init__(self, api_key: str, model_name: str):
        ...
        if self.backend == "new":
            ...
            self._legacy_model = _legacy_genai.GenerativeModel(model_name)
        else:
...
def create_gemini_model(api_key: str, model_name: str) -> Optional[GeminiModelCompat]:
    if not GEMINI_AVAILABLE or not api_key:
        return None
    try:
        return GeminiModelCompat(api_key=api_key, model_name=model_name)
    except Exception:
        return None
