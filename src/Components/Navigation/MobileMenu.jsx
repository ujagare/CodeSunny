...
const MobileMenu = () => {
  ...
  return (
    <>
      {/* Overlay */}
      <div 
        className={`fixed inset-0 bg-black bg-opacity-50 md:hidden transition-opacity duration-500 ease-in-out z-40 ${
      ...
      {/* Sidebar */}
      <div className={`fixed top-0 right-0 h-full w-full bg-gradient-to-r from-[#000000] to-[#0a0a2e] shadow-lg md:hidden transition-transform duration-500 ease-in-out z-50 ${
        ...
        <nav className="py-4" role="navigation" aria-label={navigationConfig.accessibility.ariaLabel}>
        {navigationConfig.items.map((item) => (
          <NavLink
            ...
            className={`block px-6 py-3 text-white hover:bg-white/10 transition-colors font-medium text-2xl ${
              activeMenu === item.path ? 'bg-white/20 text-white border-r-2 border-white' : ''
            }`}
